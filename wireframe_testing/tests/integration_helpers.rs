...
async fn integration_helpers_round_trip() -> TestResult {
    ...
    let result: TestResult = if response.id != ROUTE_ID {
        Err(format!("unexpected response id: {}", response.id).into())
    } else if response.payload.as_slice() != [1, 2, 3] {
        Err("response payload mismatch".into())
    } else if response.correlation_id != Some(7) {
        Err(format!(
            "expected correlation id Some(7), got {:?}",
            response.correlation_id
        )
        .into())
    } else if counter.load(Ordering::SeqCst) != 1 {
        Err("expected handler to be invoked exactly once".into())
    } else {
        Ok(())
    };
