...
| ------ | ----------                     |
| ------ | ----------                     |
| 1.1    | Core Response & Error Types    | Define the new Response\<F, E> enum with Single, Vec, Stream, and Empty variants. Implement the generic WireframeError<E> enum to distinguish between Io and Protocol errors.                        | Small  | -          |
...
| 1.3    | Connection Actor Write Loop    | Refactor the core connection.rs actor to use a select!(biased; ...) loop. This loop must poll for shutdown signals, high/low priority pushes, and the handler response stream, in that strict order. | Large  | #1.2       |
...
| 1.5    | Basic FragmentAdapter          | Implement the FragmentAdapter as a FrameProcessor. Build the inbound re-assembly logic for a single, non-multiplexed stream of fragments, and the outbound logic for splitting a single large frame. | Large  | #1.4       |
...
| ------ | ----------                            |
| ------ | ----------                            |
| 4.1    | Pervasive tracing Instrumentation     | Instrument the entire library with tracing. Add span!s for connection and request lifecycles and detailed event!s for key state transitions (e.g., back-pressure applied, frame dropped, connection terminated).                                         | Large  | All        |
...
| 4.5    | High-Quality Examples                 | Create at least two complete, high-quality examples demonstrating a real-world use case, such as a mock database protocol or an IoT message broker, combining all the new features.                                                                      | Medium | All        |
