...
*Focus: Implementing the core, non-public machinery for duplex communication
and message processing. This phase establishes the internal architecture upon
which all public-facing features will be built.*
...
| 1.5  | Basic FragmentAdapter          | Implement the `FragmentAdapter` as a codec (for example, via `tokio_util::codec`). Build the inbound reassembly logic for a single, non-multiplexed stream of fragments, and the outbound logic for splitting a single large frame. | Large  | #1.4       |
...
*Focus: Exposing the new functionality to developers through a clean,
ergonomic, and idiomatic API. This phase is about making the powerful new
mechanics usable and intuitive.*
...
| 3.2  | Re-assembly DoS Protection     | Harden the `FragmentAdapter` by adding a non-optional, configurable timeout for partial message reassembly and strictly enforcing the `max_message_size` limit to prevent memory exhaustion.         | Medium | #1.5       |
