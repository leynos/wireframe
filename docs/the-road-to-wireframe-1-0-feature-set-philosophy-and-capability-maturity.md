...
(for example, `tokio_util::codec`) that sits between the raw I/O and the main
router. It contains the generic logic for splitting large outbound frames and
re-assembling inbound fragments.
...
reassembly logic will not assume that fragments arrive sequentially. By using
