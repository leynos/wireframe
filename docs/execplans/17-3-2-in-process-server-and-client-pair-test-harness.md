...
- Decision: collapse per-field `Option`s in `WireframePair` into a single
  ...
  Correction (2026-08-17): `Drop` was subsequently changed from immediate abort
  to scheduled bounded cleanup — signal, then a detached task racing the join
  against a 100 ms grace period, aborting only on timeout; immediate abort
  remains when no runtime is current. See RFC 0001 section 5.3.
...
- Replaced the OS-thread-based `Drop` safety net (5-second sleep before
  ...
  replaced by scheduled bounded cleanup — signal, then a detached task racing
