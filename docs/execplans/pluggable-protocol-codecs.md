...
6. Propagate the codec type parameter through server types in
   `src/server/mod.rs`, `src/server/runtime.rs`, and `src/server/connection.rs`.
   Ensure factory bounds remain ergonomic with default type parameters.
...
- [ ] (2025-12-30 00:00Z) Draft ExecPlan and ADR.
- [ ] Implement `FrameCodec` and default codec module.
- [ ] Thread codec through `WireframeApp` and connection handling.
- [ ] Propagate codec type through server APIs and update examples.
...
- Decision: Follow ADR 004 proposed direction for a `FrameCodec` abstraction
  with a default length-delimited implementation.
  Rationale: Meets protocol framing needs while preserving backward
  compatibility.
