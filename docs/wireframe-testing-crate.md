...
implementation sends the shutdown signal and joins the server task with a
bounded timeout, aborting it only if it has not finished by then, as a safety
net if explicit shutdown is skipped.
...
server's accept or serve loop a reason to end, and then joins the task without
a timeout, so its `TestResult` is always observed on the explicit path. `Drop`
is a safety net for scenarios that skip or are interrupted before calling
`shutdown()`: it also signals shutdown, but bounds the join with a short
timeout and aborts only if the task has not finished by then, because `Drop`
cannot await indefinitely.
