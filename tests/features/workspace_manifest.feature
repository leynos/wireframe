Feature: Formal verification workspace manifest
  ...
  crate, the Loom models crate and the testing helper crate as explicit
  members.

  Scenario: The root manifest lists support crates without widening defaults
    ...
    And the workspace metadata includes the Loom models crate as a workspace member
    And the workspace metadata includes the testing helper crate as a workspace member
