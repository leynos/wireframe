...
impl FormalToolingWorld {
    ...
    pub fn verify_formal_execution_targets(&self) -> TestResult {
        let makefile = MakefileContent(self.loaded_makefile()?);
        verify_formal_execution_target_declarations(&makefile)?;
        verify_formal_execution_target_recipes(&makefile)?;
        verify_formal_execution_target_prerequisites(&makefile)?;
        verify_formal_execution_target_dry_runs()?;
        Ok(())
    }
...
fn verify_formal_execution_target_declarations(makefile: &MakefileContent<'_>) -> TestResult {
    for target in [
        "test-verification",
        "kani",
        "kani-full",
...
fn verify_formal_execution_target_recipes(makefile: &MakefileContent<'_>) -> TestResult {
...
fn verify_formal_execution_target_prerequisites(makefile: &MakefileContent<'_>) -> TestResult {
...
fn verify_formal_execution_target_dry_runs() -> TestResult {
    for target in ["formal-pr", "formal-nightly"] {
        ...
        if !dry_run.contains("wireframe-verification") || !dry_run.contains("formal-stub.sh") {
            return Err(format!("`make --dry-run {target}` should compose formal targets").into());
        }
    }
    Ok(())
}
