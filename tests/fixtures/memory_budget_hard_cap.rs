...
impl MemoryBudgetHardCapWorld {
    ...
    /// Validates that a server-side `io::Error` represents a budget-enforcement
    /// abort: confirms `InvalidData` kind, records the error string, drains any
    /// late payloads, and rejects non-empty payload observations.
    fn verify_abort_outcome(&mut self, error: &std::io::Error) -> TestResult {
