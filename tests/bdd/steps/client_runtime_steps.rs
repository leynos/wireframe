...
fn given_server(
    ...
    rt.block_on(client_runtime_world.start_server(max_frame_length))
}
...
fn given_client(
    ...
    let rt = tokio::runtime::Runtime::new()?;
    rt.block_on(client_runtime_world.connect_client(max_frame_length))
}
...
fn when_send_payload(client_runtime_world: &mut ClientRuntimeWorld, size: usize) -> TestResult {
    let rt = tokio::runtime::Runtime::new()?;
    rt.block_on(client_runtime_world.send_payload(size))
}
