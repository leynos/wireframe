...
impl FrameCodec for RespFrameCodec {
    ...
    fn decoder(&self) -> impl Decoder<Item = Self::Frame, Error = io::Error> + Send {
        RespAdapter::new(self.max_frame_length)
    }
...
fn parse_frame(
    ...
    match prefix {
        b'+' => parse_simple_string(buf, start),
        b':' => parse_integer(buf, start),
...
fn parse_simple_string(
    ...
    let Some((line, next)) = parse_line(buf, start + 1)? else {
        return Ok(None);
    };
...
fn parse_integer(buf: &BytesMut, start: usize) -> Result<Option<(RespFrame, usize)>, io::Error> {
    let Some((line, next)) = parse_line(buf, start + 1)? else {
        return Ok(None);
    };
    let text = str::from_utf8(line)
...
fn parse_bulk_string(
    ...
    if len == -1 {
        return Ok(Some((RespFrame::BulkString(None), next - start)));
    }
...
fn parse_array(
    ...
    let Some((line, mut cursor)) = parse_line(buf, start + 1)? else {
        return Ok(None);
    };
    let text = str::from_utf8(line)
...
fn parse_line(buf: &BytesMut, start: usize) -> Result<Option<(&[u8], usize)>, io::Error> {
    let mut index = start;
